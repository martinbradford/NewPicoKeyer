...
#include <XMLWriter.h>
#define CODE_VERSION "2022.02.08.01"
...
public:
	...
	byte read(int address) { return data[address]; };
	void write(int address, byte value) { data[address] = value; };
	void update(int address, byte value) { write(address, value); }; // no need to minimise wear on the emulation
	int length() { return sizeof(data); }
	void commit();
...
#define sdconfig_file_name "/config.ini"
...
void set_decoder_configuration(PRIMARY_SERIAL_CLS* port_to_use);
...
void serial_xml(PRIMARY_SERIAL_CLS* port_to_use);
...
void sd_card_save_eeprom_to_file(PRIMARY_SERIAL_CLS* port_to_use, String filename, bool show);
...
struct config_t {  // 120 bytes total
	...
	unsigned int magnitudelimit_low;
...
PRIMARY_SERIAL_CLS* debug_serial_port = &Serial2;
...
#ifdef DEBUG_BACKLIGHT
		debug_serial_port->printf("check_backlight %d (%d): I'm awake!\n", last_active_time, millis());
#endif //DEBUG_BACKLIGHT
...
#if defined(ARDUINO_SAMD_VARIANT_COMPLIANCE) || defined(FEATURE_MABKEYER)
				EEPROM.commit();
#endif       
...
int read_settings_from_eeprom() {
	...
	sdfile = SD.open(sdconfig_file_name);

	if (sdfile) {
		Serial.print(F("Loading eeprom from "));
		Serial.print(sdconfig_file_name);
	}
	else {
		Serial.println(F("Error opening file for reading.  Exiting."));
		return 0;
	}

	byte* p = (byte*)(&configuration);
	byte b;
...
void print_serial_help(PRIMARY_SERIAL_CLS * port_to_use, byte paged_help) {
	...
	//button_array.printall();
	//Serial.println("End of buttons");
}
#endif
//---------------------------------------------------------------------
...
void process_serial_command(PRIMARY_SERIAL_CLS * port_to_use) {
	...
	case '=':
		port_to_use->print(F("\r\nToggle Decode "));
		configuration.decode_enabled = (configuration.decode_enabled == 0 ? 1 : 0);
		config_dirty = 1;
		break;

	case '{':  set_decoder_configuration(port_to_use); break;                                              // S - Status command
...
void cli_extended_commands(PRIMARY_SERIAL_CLS * port_to_use)
{
...
#endif //FEATURE_SEQUENCER
#if defined(FEATURE_SD_CARD_SUPPORT)
	if (userinput.startsWith("SAVEEEPROM ")) { sd_card_save_eeprom_to_file(port_to_use, userinput.substring(11), true); return; }
	...
	if (userinput.startsWith("DECODE ")) {
		char* i;
		char buffer[128];
		...
		Serial.printf("New decoder parameters are %d, %d, %d\n", sf, tf, bc);

		if ((sf == 0) || (tf == 0) || (bc == 0)) {
			port_to_use->println("Parameter format error");
		}
		else {
			configuration.sampling_rate = sampling_freq = sf;
			configuration.target_frequency = target_freq = tf;
			configuration.bucket_count = bucket_count = bc;
			config_dirty = true;
			calculate_decoder_parameters();
			port_to_use->printf("Decoder parameters set\n");
			return;
		}
		// cli_keying_compensation(port_to_use, userinput.substring(5)); return; 
	}

	if (userinput.startsWith("DL ")) {
		...
		if (ml == 0) {
			...
			configuration.magnitudelimit_low = magnitudelimit_low = ml;
			config_dirty = true;
		}
		return;
	}
...
#if defined(FEATURE_SERIAL) && defined(FEATURE_COMMAND_LINE_INTERFACE) && !defined(OPTION_EXCLUDE_EXTENDED_CLI_COMMANDS) && defined(FEATURE_SD_CARD_SUPPORT) // && !defined(ARDUINO_RASPBERRY_PI_PICO)
void sd_card_load_eeprom_from_file(PRIMARY_SERIAL_CLS * port_to_use, String filename) {
	...
	if (read_settings_from_eeprom() == 0) {
		sampling_freq = configuration.sampling_rate;
		target_freq = configuration.target_frequency;
		bucket_count = configuration.bucket_count;
		magnitudelimit_low = configuration.magnitudelimit_low;
		calculate_decoder_parameters();
	}
...
void sd_card_save_eeprom_to_file(PRIMARY_SERIAL_CLS * port_to_use, String filename, bool show) {
	...
	for (x = 0; x < memory_area_end; x++) {
		...
		if (show && ((x % 16) == 0)) { port_to_use->print("."); }
	}
...
#if defined(FEATURE_SERIAL) && defined(FEATURE_COMMAND_LINE_INTERFACE) && !defined(OPTION_EXCLUDE_EXTENDED_CLI_COMMANDS) // && !defined(ARDUINO_RASPBERRY_PI_PICO)
void cli_eeprom_dump(PRIMARY_SERIAL_CLS * port_to_use) {
...
void serial_xml(PRIMARY_SERIAL_CLS * port_to_use) {
	...
	switch (configuration.keyer_mode) {
	case IAMBIC_A:  XML.writeNode("Mode", "Iambic A"); break;
	case IAMBIC_B:  XML.writeNode("Mode", "Iambic A"); break;
#ifdef FEATURE_CMOS_SUPER_KEYER_IAMBIC_B_TIMING
...
void serial_status(PRIMARY_SERIAL_CLS * port_to_use) {
	...
	port_to_use->print(F("Decode: "));
	port_to_use->println(configuration.decode_enabled ? F("On") : F("Off")); //(WD9DMP)

	port_to_use->print(F("Decoded wpm: "));
	port_to_use->println(wpm); //(WD9DMP)

	port_to_use->print(F("Decoder sidetone: "));
	port_to_use->println(decoder_sidetone ? F("On") : F("Off")); //(WD9DMP)

	port_to_use->printf("Decoder parameters are : %6.0f, %4.0f, %3.0f", trunc(sampling_freq), trunc(target_freq), trunc(bucket_count));
	port_to_use->println();
	port_to_use->printf("Decoder limits are : %d, %d", magnitudelimit, magnitudelimit_low);
	port_to_use->println();
#endif
...
#if  defined(DEBUG_BUTTONS) && defined(FEATURE_BUTTONS)
	for (int x = 0; x < analog_buttons_number_of_buttons; x++) {
		...
		port_to_use->print(button_array.getbutton(x).low_limit());
		port_to_use->print(F("  button_array_high_limit: "));
		port_to_use->println(button_array.getbutton(x).high_limit());
	}
#endif 
	//aaaaaaa
#if defined(FEATURE_ETHERNET)
...
#if defined(ARDUINO_RASPBERRY_PI_PICO) || defined (ARDUINO_MAPLE_MINI)||defined(ARDUINO_GENERIC_STM32F103C) //sp5iou 20180329
...
void initialize_keyer_state() {
	...
	configuration.decode_enabled = default_decode;
	...
	configuration.sampling_rate = default_sampling_rate;
	configuration.target_frequency = default_target_frequency;
	configuration.bucket_count = default_bucket_count;
	configuration.magnitudelimit_low = default_decoder_magnitude_limit_low;
...
#ifdef FEATURE_MABKEYER
		sd_card_load_eeprom_from_file(PRIMARY_SERIAL_PORT, sdconfig_file_name);
#endif
	}
...
#else
	debug_serial_port = SECONDARY_SERIAL_PORT; //  primary_serial_port;
#endif
...
void initializeCommandMap()
{
	...
	s_mapStringValues["reboot"] = reboot;
	s_mapStringValues["Third Value"] = evStringValue3;
	s_mapStringValues["end"] = evEnd;

}

void clientStatusCommand()
{
	StaticJsonDocument<1024> response;
	response["type"] = "status";

	switch (configuration.keyer_mode) {
		case IAMBIC_A: response["keyer_mode"] = "Iambic A"; break;
		case IAMBIC_B: response["keyer_mode"] = "Iambic B"; break;
		case BUG: response["keyer_mode"] = "Bug"; break;
		case STRAIGHT: response["keyer_mode"] = "Straight Key"; break;
		case ULTIMATIC:
			response["keyer_mode"] = "Ultimatic";
			//switch (ultimatic_mode) {
			//	// case ULTIMATIC_NORMAL:
			//	//   port_to_use->print(F("Normal")); 
			//	//   break;
			...
			//}
			break;
		case SINGLE_PADDLE: response["keyer_mode"] = "Single Paddle"; break;

		break;
	}

	if (configuration.dit_buffer_off) {
		response["dit_buffer"] = "off";
	}
	else {
		response["dit_buffer"] = "on";
	}

	if (configuration.dah_buffer_off) {
		response["dah_buffer"] = "off";
	}
	else {
		response["dah_buffer"] = "on";
	}

	if (speed_mode == SPEED_NORMAL) {
		response["speed_mode"] = "speed_normal";
		response["WPM"] = configuration.wpm;
		response["CWPM"] = configuration.wpm_command_mode;
	}
	else {
		response["speed_mode"] = "qrss";
		response["qrssditlen"] = qrss_dit_length;
	}

	switch (configuration.sidetone_mode) {
	case SIDETONE_ON: response["sidetone"] = "On"; break; //(WD9DMP)
	case SIDETONE_OFF: response["sidetone"] = "Off"; break; //(WD9DMP)
	case SIDETONE_PADDLE_ONLY: response["sidetone"] = "Paddle Only"; break;
	}
	response["sidetone"] = configuration.hz_sidetone;
	...
	response["dah_to_dit"] = float(configuration.dah_to_dit_ratio / 100);
	response["weighting"] = configuration.weighting;
	response["compensation"] = configuration.keying_compensation;
	response["serial_number"] = serial_number;
#ifdef FEATURE_POTENTIOMETER
	response["pot_wpm"] = pot_value_wpm();
	...
	port_to_use->println((float)configuration.autospace_timing_factor / (float)100);
#endif
	response["wordspace"] = configuration.length_wordspace;
	response["tx"] = configuration.current_tx;
	...
	response["qp_interrupt"] = configuration.paddle_interruption_quiet_time_element_lengths;


#if !defined(OPTION_EXCLUDE_MILL_MODE)
	if (configuration.cli_mode == CLI_NORMAL_MODE) {
		response["mill_mode"] = "off";
	}
	else {
		response["mill_mode"] = "on";
	}
#endif // !defined(OPTION_EXCLUDE_MILL_MODE)
	...
	if (configuration.ptt_buffer_hold_active) {
		...
		response["ptt_bch"] = "off";
	}

	if (configuration.ptt_disabled) {
		response["ptt"] = "disabled";
	}
	else {
		response["ptt"] = "enabled";
	}
...
#if defined(FEATURE_PADDLE_ECHO)
	if (cli_paddle_echo) {
		response["paddle_echo"] = "on";
	}
	else {
		response["paddle_echo"] = "off";
	}
	response["paddle_echo_timimg_factor"] = ((float)configuration.cw_echo_timing_factor / (float)100);
#endif


#if defined(FEATURE_STRAIGHT_KEY_ECHO)
	if (cli_straight_key_echo) {
		response["straight_key_echo"] = "on";
	}
	else {
		response["straight_key_echo"] = "off";
	}
#endif
	...
	serializeJson(response, CLIENT_SERIAL_PORT);
}
...
void service_client()
{
	...
	if (CLIENT_SERIAL_PORT.available())
	{
		char c = CLIENT_SERIAL_PORT.read();
	//	CLIENT_SERIAL_PORT.print(c);
		cmdBuffer[cmdBPointer++] = c;
		cmdBuffer[cmdBPointer] = '\0';
	//	CLIENT_SERIAL_PORT.println("Foobar2");
	//	CLIENT_SERIAL_PORT.println(s);

		StaticJsonDocument<200> doc;


		DeserializationError err = deserializeJson(doc, (const char*)cmdBuffer);
		Serial.printf("Command string is %s (%d)\n", cmdBuffer, cmdBPointer);

		if (err.code() == DeserializationError::IncompleteInput) {
			Serial.printf("incomplete input\n");
			return;
		}

		switch (err.code()) {
			case (DeserializationError::Ok): {
				Serial.printf("Looking up command\n");
				const char* cmd = doc["cmd"];

				if (cmd) {

					Serial.printf("Got command code %d\n", s_mapStringValues[cmd]);

					switch (s_mapStringValues[cmd]) {
					case status:
						Serial.println("Got status command");
						clientStatusCommand();
						break;
					...
					default:
						Serial.printf("Command %s / %d not recognised\n", cmd, s_mapStringValues[cmd]);
						break;
					}
				}
				else {
					Serial.printf("Invalid format %s\n", cmdBuffer);
				}
				break;
			}
			break;
		...
		CLIENT_SERIAL_PORT.println();
		cmdBPointer = 0;
		unsigned long end_time = micros();

		Serial.printf("command decoded and executed in %d microseconds\n", end_time - start_time);
	}

}


void service_second_core()
{
	uint32_t msg;
	if (rp2040.fifo.pop_nb(&msg)) {
		if (msg < 0x100) {
			...
			{
			case 0x101:
				if ((configuration.decode_enabled == 0) || (decoder_sidetone == 0)) return;
				tone(sidetone_line, 600);
				break;
			case 0x102:
				noTone(sidetone_line);
				break;
...
void initialize_sd_card() {
	...
	//Serial.println("Pausing to allow SD to initialize");
	//delay(3000);
...
{
	sd_card_save_eeprom_to_file(PRIMARY_SERIAL_PORT, sdconfig_file_name, false);
}
